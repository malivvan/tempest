...
func ExampleDB_Save() {
	...
	db, _ := tempest.Open(filepath.Join(dir, "tempest.db"), tempest.Codec(gob.Codec))
	defer db.Close()
...
func ExampleLimit() {
	...
	var users []User
	err := db.All(&users, tempest.Limit(2))
...
func ExampleSkip() {
	...
	var users []User
	err := db.All(&users, tempest.Skip(1))
...
func ExampleUseDB() {
	...
	db, _ := tempest.Open("", tempest.UseDB(bDB))
	defer db.Close()
